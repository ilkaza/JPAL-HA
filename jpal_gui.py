...
Classes:  
  GlobalNetwork - the Global Network of the JPAL-HA agent
  AgentNetwork - the full Agent Model
  JpalAgent - the class with all the four mechanisms of JPAL-HA: asking BTFQs, Recording, asking ATFQs, Training
                   plus the novel mechanisms of JPAL-HA: Justifications and Hypothetical Actions
"""
...
class GlobalNetwork(nn.Module):
    ...
    def __init__(self, env, USE_ADPOOL):
        ...
        if self.USE_ADPOOL:
            ...
            # footnote 6 in (Frye, 2019): The number of layers depends on the dimensions of the gridworld and are
            # chosen to take the state matrix down to 2x2
...
class JpalAgent:
    ...
    def __init__(self, env, lr=0.001, USE_JUSTIFICATIONS=True, USE_HYPOTHETICAL_ACTIONS=True, USE_SIMILARITIES=True, USE_LOCAL_NETWORK=True, USE_ADPOOL=True, USE_REAL_HUMAN=False, USE_CORRECTNESS_OF_USER_FROM_Q=True, USE_SPEECH_RECOGNITION=True, EPOCH_WEIGHT=1.5, DEFAULT_EPOCH=1, RANDOM_SAMPLING=False, LR_EPOCH_SCHEDULER=True, seed=0, USE_THYMIO=False, widget: QWidget = None):
        ...
        # list forbidden combination of 2 catastrophic actions, not to be sampled again in cases of Parenting and JPAL
        ...
        self.Janew_atfq = 0
        self.Pextra_atfq = 0
        self.Janew_dp = 0  # when gen_hypothetical_actions is called from the deadly_patterns() mechanism
        self.Pextra_dp = 0
    ...
    def get_audio(self):
        ...
        with sr.Microphone() as source:
            self.widget.pushButtonPTT.setEnabled(True)
            while not self.widget.pressed:
                if self.widget.exitFlag: os._exit(0)
                pass
            self.widget.pushButtonPTT.setEnabled(False)
            self.widget.pushButtonPTT.setText("Listening...")
    ...
    def sample_2_distinct_actions(self, state):
        ...
        actions = torch.multinomial(distr.probs, 2,
                                    replacement=False).cpu().numpy()  # don't sample the same action 2 times
        if np.all(actions == self.forbid_comb_catastr_act) or np.all(actions == np.flip(self.forbid_comb_catastr_act, 0)):  # if sampled again 2 catastrophic ones, make a random choice of two distinct actions
            ...
            while True:
                ...
                if not (np.all(actions == self.forbid_comb_catastr_act) or np.all(
                        actions == np.flip(self.forbid_comb_catastr_act, 0))):
                    break
    ...
    def get_max_action_not_in_D(self, state, pattern):
        ...
        ordered_actions_by_decreasing_probability = sorted(range(len(log_list)), key=lambda k: log_list[k],
                                                           reverse=True)
    ...
    def get_explor_action(self, state, random):
        ...
        """
        max_action = self.get_max_action(state)  # No help from parent so it's probably safe: EXPLORE
        if random == False:
    ...
    def get_explorative_action_not_in_D(self, state, pattern, random):
        ...
        max_action = self.get_max_action(
            state)  # No help from parent so it's probably safe: EXPLORE - a catastrophe might get in here
    ...
    def give_preference_without_warning_from_Q(self, q1, q2):
        """
        Returns preference of human according to q-values when justification is 'no warning'
    ...
    def check_bothering(self, state, actions, entries_before_X_end):
        ...
        for i in range(len(self.X[0]) - entries_before_X_end):
            index += 1
            if self.USE_LOCAL_NETWORK:
                if (np.array_equal(self.X[0][i][0], st_global) and self.X[1][i] == actions[0] and self.X[2][i] ==
                    actions[1]) \
                        or (
                        np.array_equal(self.X[0][i][0], st_global) and self.X[1][i] == actions[1] and self.X[2][i] ==
                        actions[0]):
                    bothered = True
                    break
            else:
                if (np.array_equal(self.X[0][i], st_global) and self.X[1][i] == actions[0] and self.X[2][i] == actions[
                    1]) \
                        or (np.array_equal(self.X[0][i], st_global) and self.X[1][i] == actions[1] and self.X[2][i] ==
                            actions[0]):
                    bothered = True
                    break
    ...
    def print_board(self, state):
        ...
        data = self.convert2board(state['board'])
        
        #pyside6 unfortunately destroys the alignment of the board when the console output is sent to the GUI. This makes it somewhat better but won't work for other configurations.
        data = data.tolist()
    ...
    def give_justification_one_action_from_real_human(self, state, parent, action, x, y):
        ...
        if self.USE_SPEECH_RECOGNITION:
            self.widget.pushButtonWR.setStyleSheet("color: black; background-color: green")
            self.widget.pushButtonNW.setStyleSheet("color: black; background-color: green")
            while True:
                justification = self.get_audio()
                if justification in ['warning', 'no warning']:
                    if self.USE_CORRECTNESS_OF_USER_FROM_Q:
                        if justification == sim_justification:
                            break
                        else:
                            print("Are you sure you gave the right answer? Please try again!")
                            self.Janew_misses +=1
                    else:
                        break
            self.widget.pushButtonWR.setStyleSheet("")
            self.widget.pushButtonNW.setStyleSheet("")
        else:
            while True:
                self.widget.pushButtonWR.setEnabled(True)
                self.widget.pushButtonNW.setEnabled(True)
                while not self.widget.pressed:
                    if self.widget.exitFlag: os._exit(0)
                    pass
                self.widget.pressed = False
                if self.widget.warning_action:
                    justification = 'w'
                else:
                    justification = 'e'
                if justification in ['w', 'e']:
                    ...
                    if self.USE_CORRECTNESS_OF_USER_FROM_Q:
                        ...
                        break
        toc = time.time()
        self.widget.pushButtonWR.setEnabled(False)
        self.widget.pushButtonNW.setEnabled(False)
        self.Janew_time.append(toc-tic)
        return justification

    def give_preference_parenting_from_real_human(self, state, actions, parent, x, y):
        """
        ...
        tic = time.time()
        if self.USE_SPEECH_RECOGNITION:
            self.widget.pushButtonFO.setStyleSheet("color: black; background-color: green")
            self.widget.pushButtonSO.setStyleSheet("color: black; background-color: green")
            self.widget.pushButtonEI.setStyleSheet("color: black; background-color: green")
            self.widget.pushButtonNEI.setStyleSheet("color: black; background-color: green")
            while True:
                preference = self.get_audio()
                preference = self.correct_speech(preference)
                if preference in [self.convert2move(actions[0]), self.convert2move(actions[1]), 'either', 'neither']:
                    preference = self.move2pref(preference, actions)
                    if self.USE_CORRECTNESS_OF_USER_FROM_Q:
                        ...
                        break
            self.widget.pushButtonFO.setStyleSheet("")
            self.widget.pushButtonSO.setStyleSheet("")
            self.widget.pushButtonEI.setStyleSheet("")
            self.widget.pushButtonNEI.setStyleSheet("")
        else:
            self.widget.pushButtonFO.setEnabled(True)
            self.widget.pushButtonSO.setEnabled(True)
            self.widget.pushButtonEI.setEnabled(True)
            self.widget.pushButtonNEI.setEnabled(True)
            while True:
                while not self.widget.pressed:
                    if self.widget.exitFlag: os._exit(0)
                    pass
                self.widget.pressed = False
                preference = str(self.widget.choice_action)
                if preference in ['1', '2', '3', '4']:
                    preference = help_dict_parenting[preference]
                    if self.USE_CORRECTNESS_OF_USER_FROM_Q:
                        if preference == sim_preference:
                            break
                        else:
                        ...
                        break
        toc = time.time()
        self.widget.pushButtonFO.setEnabled(False)
        self.widget.pushButtonSO.setEnabled(False)
        self.widget.pushButtonEI.setEnabled(False)
        self.widget.pushButtonNEI.setEnabled(False)
        
        self.P_time.append(toc-tic) # Parenting             
        return preference

    def give_preference_without_warning_from_real_human(self, state, actions, parent, x, y):
        """
        Returns preference according to real human input when justification is 'no warning' - used in JPAL-HA
        ...
        if self.USE_SPEECH_RECOGNITION:
            self.widget.pushButtonFO.setStyleSheet("color: black; background-color: green")
            self.widget.pushButtonSO.setStyleSheet("color: black; background-color: green")
            self.widget.pushButtonNP.setStyleSheet("color: black; background-color: green")
            while True:
                preference = self.get_audio()
                preference = self.correct_speech(preference)
                if preference in [self.convert2move(actions[0]), self.convert2move(actions[1]), 'no preference']:
                    preference = self.move2pref(preference, actions)
                    if self.USE_CORRECTNESS_OF_USER_FROM_Q:
                        if preference == sim_preference:
                            break
            ...
            self.widget.pushButtonFO.setStyleSheet("")
            self.widget.pushButtonSO.setStyleSheet("")
            self.widget.pushButtonNP.setStyleSheet("")
        else:
            self.widget.pushButtonFO.setEnabled(True)
            self.widget.pushButtonSO.setEnabled(True)
            self.widget.pushButtonNP.setEnabled(True)
            while True:
                while not self.widget.pressed:
                    if self.widget.exitFlag: os._exit(0)
                    pass
        ...
        self.widget.pushButtonFO.setEnabled(False)
        self.widget.pushButtonSO.setEnabled(False)
        self.widget.pushButtonNP.setEnabled(False)

        self.Pextra_time.append(toc-tic)             
        return preference
    ...
    def give_justification_from_real_human(self, state, actions, parent, x, y):
        ...
        tic = time.time() 
        if self.USE_SPEECH_RECOGNITION:
            print("Please give alarm signal")
            self.widget.pushButtonWR.setStyleSheet("color: black; background-color: green")
            self.widget.pushButtonNW.setStyleSheet("color: black; background-color: green")
            while True:
                justification = self.get_audio()
    ...
    def give_preference_justification_from_real_human(self, state, actions, parent, x, y):
        """
        Returns preference and justification according to real human input in the general case of JPAL(-HA)
        ...
        self.widget.pushButtonWR.setEnabled(False)
        self.widget.pushButtonNW.setEnabled(False)
        self.P_time.append(toc_p-tic_p)
        self.Jp_time.append(toc_j - tic_j)        
        return preference, justification

    def move2pref(self, preference, actions):
        """
        Convert a move (direction) to a preference (first, second) - in Parenting it might stay as 'either' or 'neither'
        
        :param preference: preference between the two actions
        :param actions: int two candidate actions        
        :returns: string preference between actions      
        """   
        if preference == self.convert2move(actions[0]):
            preference = 'first'
        elif preference == self.convert2move(actions[1]):
            preference = 'second'
        elif preference == 'no preference':
            preference = 'equal'
        return preference

    def correct_speech(self, preference):
        """
        Correct similar words that speech recognition system will often confuse
        
        :param preference: preference between the two actions     
        :returns: string preference between actions      
        """ 
        if preference in ['no', 'not', 'none', 'known', 'notes', 'laugh', 'nice', 'nose', 'Noel', 'not']:
            preference = 'north'
        if preference in ['so', 'soul', 'Sal', 'Sam', 'son', 'spouse']:
            preference = 'south'
        return preference
    ...
    def is_clip_in_memory(self, state, action1, action2, mem):
        ...
        if mem == 'X':
            ...
            raise Exception("mem should be 'X', 'R' or 'XR'")

    def generate_hypothetical_actions(self, preference, actions, state, parent, x, y, counter_actions_X5, query_origin):
        """
        ...
        :param query_origin: string 'btfq', 'aftq' or 'deadly_pattern' according to where is the function called from
        :returns: action to be taken
        """
        if preference == 'equal':
            safe_actions = []
            catastrophic_actions = [x for x in
                                    actions]  # in the case of a dangerous pattern detected from the beginning, it could constitute of more than two actions
        ...
        while len(safe_actions) < 2 and len(safe_actions) + len(catastrophic_actions) < NUM_ACTIONS:
            ...
            if query_origin == 'btfq':
                self.Janew_btfq += 1
            elif query_origin == 'atfq':
                self.Janew_atfq += 1
            elif query_origin == 'deadly_pattern':
                self.Janew_dp += 1
            else:
                raise Exception("query_origin must be one of 'btfq', 'atfq' 'deadly_pattern' ")
            ...
            if justification_new_action == 'warning':
                ...
                for safe in safe_actions:
                    ...
                    if bothered:
                        ...
                        if query_origin == 'btfq':
                            ...
                            raise Exception("query_origin must be one of 'btfq', 'deadly_pattern' or 'deadly_pattern' ")
                        if self.USE_REAL_HUMAN:
                ...
                safe_actions.append(
                    new_action)  # currently it was not needed like this. Could be used in case a more complex condition between the number of safe and catastrophic actions
        self.X[5].extend([
                             action] * counter_actions_X5)  # for all those combinations, the action to be taken if a query about those is asked, is the best found action from that state
        return action
    ...
    def ask_BTFQ(self, state, parent, client):
        ...
        sample_again = True
        st_global = self.convert2ohe(state['board'])
        while sample_again:  # may sample again only in the Parenting or just JPAL cases
            sample_again = False
            if self.USE_LOCAL_NETWORK:
                st_local = self.convert2local(state)
            ...
            while bothered:
                ...
                if staying or returning:
                    ...
                    if not self.RANDOM_SAMPLING:
                        ...
                        new_action = self.sample_non_greedy_random_action(
                            checked_actions)  # unstuck from cases of 3 equal actions, when the 4th is the correct, e.g. (r,c)=(3,4) quicker
            ...
            if self.USE_SIMILARITIES and self.USE_HYPOTHETICAL_ACTIONS:
                ...
                sort_D.sort(key=len, reverse=True)
                for pattern in sort_D:
                    if [st_local[x] for x in pattern] == [self.water_char] * len(
                            pattern):  # attention, the pattern can be more than 2 actions!
                        ...
                        for comb in itertools.combinations(pattern, 2):
                            if not self.is_clip_in_memory(state, comb[0], comb[1], mem='X'):
                                ...
                                counter_actions_X5 += 1
                        action = self.generate_hypothetical_actions('equal', pattern, state, parent, x, y,
                                                                    counter_actions_X5, 'deadly_pattern')
                        response_of_env = self.wrap_up_step(state, action, client)
                        return response_of_env
            ...
            if self.USE_JUSTIFICATIONS:
                ...
                if justification == 'no warning':
                    ...
                    if self.USE_HYPOTHETICAL_ACTIONS:
                        counter_actions_X5 = 1
                        action = self.generate_hypothetical_actions(preference, actions, state, parent, x, y, counter_actions_X5, 'btfq')
                ...
                self.P_btfq += 1
                if self.USE_REAL_HUMAN:
                    preference = self.give_preference_parenting_from_real_human(state, actions, parent, x, y)
    ...
    def record(self, state):
        ...
        explorative_action = self.get_explorative_action(state, random=True)  # 3          
        ...
        self.recordings += 1  # and it saves [0, None] here
    ...
    def ask_ATFQ(self, parent):
        ...
        if self.USE_REAL_HUMAN: print("Time for an ATFQ...")
        if self.USE_THYMIO:
        ...
        if self.USE_JUSTIFICATIONS:
            ...
            if justification == 'no warning':
                action = self.pick_action_without_warning([exploitative_action, explorative_action],
                                                          preference)  # we don't take the action - just checking for X[5]
                if self.USE_HYPOTHETICAL_ACTIONS: self.X[5].append(action)
            elif justification == 'warning':
                if self.USE_HYPOTHETICAL_ACTIONS:  # we don't care about the action in ATFQs but we still need to check the surrounding area if we see a catastrophe
                    counter_actions_X5 = 1
                    _ = self.generate_hypothetical_actions(preference, [exploitative_action, explorative_action], state, parent, x, y, counter_actions_X5, 'atfq')
        else:
            self.P_atfq +=1
            if self.USE_REAL_HUMAN:
                preference = self.give_preference_parenting_from_real_human(state, [exploitative_action, explorative_action], parent, x, y)
                if preference in ['either', 'neither']:
                    self.X[3].append(0.5)
                elif preference == 'first': 
                    self.X[3].append(1)
                elif preference == 'second': 
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            ...
            pi1Prob = distr.probs.gather(1, act1.view(-1,
                                                      1))  # these 2 probs=pis don't sum to 1 as there are 2 more actions
            ...
            loss = bce((pi1Prob / (pi1Prob + pi2Prob)).squeeze(1),
                       targetProb)  # Output: pi1/(pi1+pi2), target: μ in [0-1]
