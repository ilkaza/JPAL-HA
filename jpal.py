...
def get_audio():
    ...
    with sr.Microphone() as source:
        input("Press Enter and talk when ready")
        print("listening...")
...
class JpalAgent():
    ...
    def __init__(self, env, lr=0.001, USE_JUSTIFICATIONS=True, USE_HYPOTHETICAL_ACTIONS=True, USE_SIMILARITIES=True, USE_LOCAL_NETWORK=True, USE_ADPOOL=True, USE_REAL_HUMAN=False, USE_CORRECTNESS_OF_USER_FROM_Q=True, USE_SPEECH_RECOGNITION=True, EPOCH_WEIGHT=1.5, DEFAULT_EPOCH=1, RANDOM_SAMPLING=False, LR_EPOCH_SCHEDULER=True, seed=0, USE_THYMIO=False):
        ...
        :param USE_LOCAL_NETWORK: boolean to additionally use adds the local network
        :param USE_ADPOOL: boolean for adaptive max pooling for the Global Network
    ...
    def get_explorative_action_not_in_D(self, state, pattern, random):
        ...
        :returns: int explorative action or max_action if none else was sampled
        """  
        max_action = self.get_max_action(state) # No help from parent so it's probably safe: EXPLORE - a catastrophe might get in here       
    ...
    def map_to_mu(self, preference, justification):
        """
        Maps input from human (preference and justification) to mu in in the general case of JPAL(-HA)
    ...
    def give_justification_one_action_from_real_human(self, state, action):
        ...
        print ("Please give alarm signal if Action=" + self.convert2move(action).capitalize() + " had been taken")
        tic = time.time()                   
        if self.USE_SPEECH_RECOGNITION:        
            print ("Warning or No Warning")
            while True:
            ...
            while True:                
                justification = input("Warning(w), No warning(e) \n")
    ...
    def give_preference_parenting_from_real_human(self, state, actions, parent, x, y):
        ...
        tic = time.time()          
        if self.USE_SPEECH_RECOGNITION:              
            print(self.convert2move(actions[0]).capitalize() + ", " + self.convert2move(actions[1]).capitalize() + ", Either" + " or Neither")
            while True:
            ...
            while True:                
                preference = input(self.convert2move(actions[0]).capitalize() + "(1), " + self.convert2move(actions[1]).capitalize() + "(2), Either(3) or Neither(4)\n")
    ...
    def give_preference_without_warning_from_real_human(self, state, actions, parent, x, y):
        ...
        tic = time.time()          
        if self.USE_SPEECH_RECOGNITION:              
            print(self.convert2move(actions[0]).capitalize() + ", " + self.convert2move(actions[1]).capitalize() + " or No Preference")
            while True:
                preference = get_audio()
                preference = self.correct_speech(preference)
            ...
            while True:                
                preference = input(self.convert2move(actions[0]).capitalize() + "(1), " + self.convert2move(actions[1]).capitalize() + "(2), or No Preference(3) \n")
    ...
    def give_preference_justification_from_real_human(self, state, actions, parent, x, y):
        ...
        if self.USE_SPEECH_RECOGNITION:
            ...
            while True:
                preference = get_audio()
                preference = self.correct_speech(preference)
                #if preference in ['first', 'second', 'no preference']:
                if preference in [self.convert2move(actions[0]), self.convert2move(actions[1]), 'no preference']:
                    preference = self.move2pref(preference, actions)
                    if self.USE_CORRECTNESS_OF_USER_FROM_Q:
                        if preference == sim_preference:
    ...
    def generate_hypothetical_actions(self, preference, actions, state, parent, x, y, counter_actions_X5, query_origin):
        ...
        while len(safe_actions)<2 and len(safe_actions)+len(catastrophic_actions)<NUM_ACTIONS:
            ...
            if query_origin == 'btfq':
                ...
                self.Janew_dp +=1
            else: raise Exception("query_origin must be one of 'btfq', 'atfq' 'deadly_pattern' ")            
                ...
                for safe in safe_actions:
                    ...
                    if bothered:
                        ...
                        if query_origin == 'btfq':
                            ...
                            self.Pextra_dp +=1
                        else: raise Exception("query_origin must be one of 'btfq', 'deadly_pattern' or 'deadly_pattern' ")                                           
                        if self.USE_REAL_HUMAN:
                    ...
                    if [st_local[x] for x in pattern] == [self.water_char]*len(pattern): # attention, the pattern can be more than 2 actions!
                        ...
                        for comb in itertools.combinations(pattern, 2):
                            if not self.is_clip_in_memory(state, comb[0], comb[1], mem='X'):
                                ...
                                counter_actions_X5 +=1
                        action = self.generate_hypothetical_actions('equal', pattern, state, parent, x, y, counter_actions_X5, 'deadly_pattern') 
                        response_of_env = self.wrap_up_step(state, action, client)
                        return response_of_env
